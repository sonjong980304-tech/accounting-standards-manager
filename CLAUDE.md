...
## 역분석 결과 (List016005 기준 — requests로 100% 재현 가능, Playwright 불필요)
...
- **3층 신뢰 UI**: ① 답변+인용 하이라이트(배지 클릭→근거카드 앵커 스크롤) ② 근거 원문 카드
  ...
  · **PDF 페이지 렌더 폐기**: PyMuPDF로 원본 PDF p.N 이미지 렌더를 넣었으나
...
- **문단 exact 실패 원인 분류(dump 1,956건 자동)**: 표기차는 정규화로 흡수 → **진짜 검색 실패
  ...
  · 7.4% ±1 인접. **27%는 채점 아티팩트가 아닌 실제 검색 성능**. 답변 품질 영향 제한적(호 79.1% 커버).
- **향후 검색 개선 로드맵(미구현 — 문서화만)**:
  · **문단 정밀도**(실패 59%): per_coll·k 상향, 리랭커 강화, 문단 청킹 재검토. 예상효과=문단 exact
    ...
    미검증**(이미 호 recall 79.1%가 커버하는 영역) → **우선순위 뒤로**.
...
- **리랭커 max_length 절단 가설 검증 — 512 유지 결정 (2026-07-26)**:
  - **가설**: `reranker.predict((query, doc))`는 질문+문서를 합쳐서 512토큰으로 자르는데(HF
    `longest_first`, 뒤쪽부터 제거), 다중 논제 질의(예: (a)~(g) 하위질문)는 질문 혼자만으로도
    512를 넘는 경우가 있어(골든셋 1,195건 중 **42건·3.5%**, median 110·p90 340·p99 702·max 1231
    토큰) 뒷부분 논제가 리랭커에서 통째로 안 보일 수 있음 → recall 저하 우려.
  - **실측**(이 42건만 골라 `Index()` 1회 로드 후 `index.reranker`만 `max_length=512`(현행) vs
    `1024`로 교체해 재검색+리랭킹, 같은 dense/BM25 후보로 A/B):
    - top-5: 전 지표 개선(문단 hit rate 9.5%→14.3%, 호 recall 50.8%→54.0%).
    - top-10: 오히려 소폭 하락(문단 hit rate 19.0%→16.7%, 호 recall 67.7%→66.5%).
    - top-10 기준 회수 문단 집합이 바뀐 6/42건도 512가 이긴 3건·1024가 이긴 2건·무승부 1건 —
      "늘리면 무조건 좋다"를 지지하지 않음.
    - 비용: 42건 리랭킹 257s→780s(**~3배** 느려짐).
  - **결론**: 가설은 이 표본에서 뚜렷이 확인되지 않음. 이 42건 자체의 recall이 낮은 건(달성가능
    대비 top-10 4.1%) truncation보다 **정답 문단이 4~9개(다중 인용)인데 top-5/10 슬롯이 좁은
    구조적 한계**가 더 유력한 원인. **`max_length=512` 유지** — 속도 손해(~3배) 대비 recall 개선
    근거가 불충분.
  - **재검증 방법(재사용 가능)**: 질문 토큰 길이는 `AutoTokenizer.from_pretrained(RERANK_MODEL)`로
    측정. A/B는 `Index()`를 한 번만 만들고 `index.reranker`를 다른 `max_length`의 `CrossEncoder`로
    교체해 재리랭킹하면, 임베더·Chroma·BM25 재로드 없이 리랭커 설정만 비교 가능.
...
- **B 실검증 상태**: `judge.py` **OpenAI 경로 실 API 파싱 성공**(스텁 아님) — 근거충실
...
## answer has_grounds 이중 기준 — 로컬 완화 / GPT 엄격
...
## 라우팅·검색 컬렉션별 보장  — 개발비 refusal 버그 수정
...
- **배치 평가 무영향**: 배치(`run_batch`)는 **라우터 미사용** + `min_standards=0`으로 새 보장 로직을
  스킵 → recall 수치(문단 exact 0.267·호 0.791 등) 불변. 이번 수정은 실시간 앱 경로만 변경.
- **남은 근본 한계**: ③ 리랭커의 기준서 문단 저평가는 미해결(향후 개선 로드맵 참조). 이번 수정은
  "정답 컬렉션이 라우팅·슬롯에서 통째로 누락되던" 문제를 막은 것이지, 리랭커 순위 자체는 못 올림.

## rewrite 질의 정규화 — 표면형 민감 refusal 잔존분 수정
- **증상**: 위  수정 후에도 "개발비**의** 자산인식요건 **알려줘**"는 GPT가 여전히
...
## 역분석 결과 — 기준서 게시판 (ingAccountingList.do)
...
- 문단번호 패턴: `문단\s*((?:[A-Z]{1,2})?\d+(?:\.\d+)*[A-Z]*(?:[⑴-⒇])?)`
  ...
  ※ 원문자는 ⑴~⒇ (확장: 제1001호 문단 54가 ⒅까지 실사용 확인)
...
### 용어정의 추출 (extract_term_records) 엣지케이스 처리 
