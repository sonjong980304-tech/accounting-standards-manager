...
>
> ※ 위 표는 2026-07-04 측정 스냅샷입니다. LLM 특유의 비결정성 때문에 재실행하면 정확한 점수는 달라질 수 있습니다 — 실제로 같은 두 질문(파생상품·틀린 전제)을 오늘(2026-07-13) 여러 번 다시 돌려보니 더 이상 낮은 점수가 재현되지 않았습니다(파생상품은 매번 동일한 F=1.0, 틀린 전제는 매번 refusal). 그래서 질문을 다양화해 다시 낮은 점수가 나오는 사례를 찾았고, 아래에는 그렇게 **실제로 재현된 저점수 사례**를 그대로 남깁니다(점수가 높게 나온 케이스로 대체하지 않음).

#### 실제 사례 — EXAONE의 낮은 Faithfulness 재현 검증 (2026-07-13)

*"전환사채와 신주인수권부사채의 회계처리 차이는 무엇인가?"* 질문을 EXAONE 경로로 실행했더니, 판사(OpenAI/gpt-4o-mini)가 **Faithfulness 0.5**(Answer Relevancy는 1.0)를 매겼습니다. 이번 사례는 위에서 언급한 두 유형(자체 지식 문단번호 인용·틀린 전제 미교정) 중 어느 쪽도 아니라, **근거를 인용하긴 했지만 그 인용에 근거 없는 결론을 덧붙이는** 세 번째 유형입니다 — Faithfulness가 낮아지는 원인이 한 가지가 아님을 보여줍니다.
...
이 지적이 실제로 맞는지 DB 원문과 직접 대조했습니다.

- **문장 1은 뜻밖에도 판사가 과하게 지적한 것으로 보입니다.** `016005-40600`의 실제 원문에는 "시장참여자가 전환사채의 가격을 결정하는 데에 고려하는 요소(시간 포함)의 변동을 반영하여 손익으로 인식함(제1109호 문단B5.1.2A)... 합리적 배분기준을 적용하여(예: 정액법, 유효이자율법) 상각해야 함"이라고 되어 있어, EXAONE의 문장과 내용이 크게 다르지 않습니다. 즉 **판사도 실제로는 근거가 있는 문장을 "근거 없음"으로 오판할 수 있습니다** — 앞서 설명한 "판사가 관대해서 놓치는 경우"뿐 아니라 **"판사가 과해서 잘못 짚는 경우"도 실제로 관찰됩니다.**
