...
> ⏳ **처음 접속하거나 첫 질문을 던질 때는 1~2분 정도 걸릴 수 있습니다.** 무료 호스팅(Hugging Face Spaces)이라 앱이 한동안 쉬면 절전 상태로 들어가 다시 깨어나는 시간이 필요하고, 첫 질문 순간에 임베딩·리랭커 모델(BGE-M3 · bge-reranker-v2-m3, 합계 약 2.5GB)과 벡터DB(약 250MB)를 처음 한 번 내려받기 때문입니다. **두 번째 질문부터는 이 다운로드가 반복되지 않아 첫 질문보다는 빠릅니다.**
>
> ⚠️ **다만 두 번째 질문부터도 로컬보다는 느립니다 — 배포판은 CPU로 구동되기 때문입니다.** 위 다운로드 지연은 최초 1회성이지만, Hugging Face Spaces 무료 티어는 GPU가 없어 **매 질문마다** 임베딩·리랭킹 연산을 CPU로 처리합니다. 이건 캐싱으로 없앨 수 있는 지연이 아니라 질문마다 반복되는 근본적인 속도 차이라, 응답 속도가 로컬 실행(GPU/MPS)보다 계속 느리고 Space가 할당된 서버 사양에 따라서도 달라질 수 있습니다. 빠른 응답이 필요하면 저장소를 클론해 로컬(특히 Apple Silicon 등 GPU/MPS 가속 가능한 환경)에서 실행하는 것을 권장합니다.
