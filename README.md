...
>
> ⚠️ **주의: 배포판은 CPU로 구동됩니다.** Hugging Face Spaces 무료 티어는 GPU가 없어, 질문마다 임베딩·리랭킹 연산을 CPU로 처리합니다. 그래서 **응답 속도가 로컬 실행(GPU/MPS)보다 느리고, Space가 할당된 서버 사양에 따라 체감 속도가 달라질 수 있습니다.** 빠른 응답이 필요하면 저장소를 클론해 로컬(특히 Apple Silicon 등 GPU/MPS 가속 가능한 환경)에서 실행하는 것을 권장합니다.
