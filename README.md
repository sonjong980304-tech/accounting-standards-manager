...
- **개발비 자산인식 질문 비결정적 refusal 버그**: "개발비 자산인식요건"을 물으면 GPT가 근거 카드는 정상 표시하면서도 답변만 "근거를 찾지 못했습니다"로 실패하는 현상이 있었습니다. 원인은 라우팅 비결정성·컬렉션당 최소보장 부재·리랭커의 기준서 문단 저평가(질의회신 대비 0.9 vs 0.1) 세 가지가 겹친 것이었고, 셋을 모두 고쳐 재현 3/3 정상화했습니다.
- **구어체 질문 검색 실패 — rewrite 정규화 추가**: "개발비의 자산인식요건 알려줘"처럼 붙여쓰거나 구어체인 질문은 정답 문단의 리랭커 점수가 0.608에서 0.012로 급락해 검색에서 통째로 탈락했습니다. 매 질의를 검색 친화적으로 다시 쓰는 rewrite 단계를 추가해 점수를 0.576까지 회복시켰습니다.**(오픈소스 리탱커 모델의 한계)**
- **recall 수치 자체가 부풀려져 있던 버그 발견 및 수정**: 초기 채점 로직이 하위항목 원문자(⑴⑵⑷)를 제거해 서로 다른 문단을 같은 것으로 합쳐버려, 실제로는 그렇지 않은데 recall이 26.7%→30.1%로 부풀려져 있었습니다. NFKC 정규화 방식으로 교정해 충돌 0건을 확인했습니다.
