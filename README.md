...
LangGraph 5개 노드가 질문을 처리하고 audit_lookup이 참고 정보(감리지적사례)를 추가합니다. 검색은 4개 컬렉션(기준서 2 + 질의회신 2, 전부 KASB 자료)으로 나눠 라우터가 필요한 곳만 고르게 했고 SQLite 체크포인터로 대화 맥락을 이어갑니다. 점선으로 표시한 경로는 답변 근거로 쓰이지 않는 **참고 전용** 흐름입니다.
...
**1. rewrite** (질문 재작성)
 **• 목적:** 사용자의 질문을 검색 엔진이 가장 이해하기 쉬운 형태의 키워드로 다듬는 단계입니다. (기본 엔진: gpt-4o-mini / 로컬 모드: EXAONE)
 **• 동작 조건:**
   조건 A (대화 이력이 있는 경우): ⁠route⁠ 노드 진입 전에 무조건 실행합니다. (이전 대화 맥락 해소는 검색 점수와 무관하게 항상 필요하기 때문)
   조건 B (첫 질문인 경우): 재작성 없이 즉시 ⁠route⁠ → ⁠retrieve⁠를 먼저 시도합니다. 이후 1차 검색 결과의 랭킹 1위(top) 리랭커 점수가 0.6 미만일 때만 ⁠rewrite⁠    → ⁠retrieve⁠ 경로로 재시도합니다. (1회 한정으로 동작하여 무한 루프를 방지하며, 재시도 후에도 점수가 낮으면 그대로 ⁠answer⁠ 노드로 진입합니다.)

**2. route** (검색 방향 설정)
 **• 목적:** 질문 속에서 회계 기준 신호(K-IFRS 또는 일반기업회계기준)를 포착하여 검색할 데이터베이스 컬렉션을 선택합니다. (로컬 모드에서는 EXAONE 엔진이 판단을 처리)
 **• 동작 방식:** 만약 어떤 기준서인지 신호가 모호하다면 섣불리 한쪽으로 좁히지 않습니다. 정답 컬렉션을 통째로 놓치는 치명적인 오류를 막기 위해, 양쪽 기준서 및 질의회신 컬     렉션을 모두 검색하도록 분기를 열어둡니다.

**3. retrieve** (하이브리드 검색 및 리랭킹)
 **• 목적:** 라우팅된 컬렉션에서 질문과 가장 밀접한 문서들을 찾아 정렬합니다.
 **• 핵심 로직:** 라우팅 대상이 된 각 *_standards 컬렉션에서 최소 1건의 문서 확보를 보장합니다.
 **• 보장 이유:** 리랭커 모델의 특성상 구조적으로 질의회신(Q&A) 데이터를 기준서 문단보다 훨씬 높게 평가하는 편향이 존재합니다. 단순히 종합 점수 순으로만 top-k를 잘라버리면 정답인 기준서 원문이 뒤로 밀려 통째로 빠지는 현상이 발생합니다.
 **• 실제 발생했던 버그:** 개발비 자산인식요건" 질문 당시, 정답 문단이 4개 컬렉션 합산 리랭킹 17위까지 밀려나면서 GPT가 답변을 거부(refusal)하는 문제가 있었습니다. 이를 해결하기 위해 각 기준서 컬렉션별 최소 1개 노출 룰을 강제합니다.

 **4. audit_lookup** (감리지적사례 교차 조회)
 **• 목적:** 메인 retrieve 과정과 완전히 분리된 데이터인 금융감독원 감리지적사례(별도 audit_cases 컬렉션, 총 65건)'를 조회 하는 단계입니다. ( route 단계를 마친  후 실행됩니다.)
 **• 제약 제어:** 이 노드의 조회 결과는 answer 노드에서 답변을 작성하는 직접적인 근거로 절대 사용되지 않습니다.
 **• 노출 조건:** 리랭커 점수가 0.6 이상인 관련 사례가 발견되는 경우에만, 최종 답변과 나란히 배치하여 사용자가 비교해 볼 수 있는 '참 고 정보' 형태로만 화면에 제공합니다.

 **5. answer** (근거 기반 답변 생성)
 **• 목적:** 사용자에게 최종 답변을 작성하여 전달합니다.
 **• 제약 조건:** 외부 지식을 배제하고 오직 검색( retrieve )을 통해 찾아온 확실한 근거 문서만을 사용하여 답해야 합니다.
 **• 인용 규칙:** 답변 문장 내에 근거 식별자를 명확하게 삽입해야 합니다. (예시: ..라고 규정하고 있습니다 [제1116호 문단 7]. )

 **6. verify** (원문 교차 검증)
 **• 목적:** AI가 답변 과정에서 출처를 왜곡하거나 없는 문장을 만들어내는 환각 현상(Hallucination)을 원천 차단합니다.
 **• 동작 방식:** answer 노드가 답변에 인용한 근거(예: [제1116호 문단 7] )를 바탕으로, DB에서 실제 원문을 있는 그대로 조회하여 유저에게 반환합니다. 이 과정에서 LLM을 이용한 텍스트 재생성은 엄격히 금지됩니다.
