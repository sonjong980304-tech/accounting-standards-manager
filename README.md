...
**Problem** — 회계기준은 K-IFRS와 일반기업회계기준 두 체계로 나뉘고, 같은 주제라도 서로 다른 기준서 문단에서 다룹니다. 실무자는 "이 거래를 어느 기준의 어느 문단으로 처리하나"를 찾는 데 많은 시간을 씁니다. 일반 LLM에 그대로 물으면 문단 번호를 그럴듯하게 지어내거나(환각) 두 체계를 뒤섞기 쉬워, 회계처럼 출처 확인이 곧 신뢰인 도메인에서는 그대로 쓰기 어렵습니다.

**Analyze** — 청킹 단위를 고정 크기로 분할할지, 이미 문단·용어정의·질의회신 Q&A로 정제된 원본 단위를 그대로 쓸지 검토했고, 근거 인용 단위(문단 번호)와 어긋나지 않도록 후자를 택했습니다. 검색도 BM25(어휘)만 쓸지 dense(의미) 임베딩만 쓸지 저울질했지만, 두 방식이 놓치는 지점이 서로 달라 하이브리드로 함께 쓰는 쪽을 택했습니다. 리랭커 모델도 벤치마크 최고점 모델(Gemma2-9B 기반, fp16 기준 약 18GB)과 실제 서빙 환경(맥미니, 통합메모리 16GB) 제약을 저울질해, 로컬 구동이 가능한 568M 모델(bge-reranker-v2-m3)로 결정했습니다.
...
**Result** — 골든셋 1,192건 기준 검색 성능은 호(기준서) 단위 recall **79.1%**(top-10), 정답 문단을 1건이라도 회수하는 hit rate **38.6%**를 기록했습니다. 생성 품질은 GPT 경로 기준 단기리스·파생상품처럼 근거가 명확한 질문에서 Faithfulness·Answer Relevancy가 모두 1.0을 기록했고, 근거가 없는 질문(국내 기준과 무관한 미국세법 질문)은 GPT·로컬 모델 모두 안정적으로 답변을 거부(refusal)해 환각 방지 장치가 실제로 작동함을 확인했습니다.
...
위 4개 질문을 다시 실행하며 각 노드(검색범위 분류 `route`, 답변 생성 `answer`)가 실제로 소비한 토큰을 `usage_metadata`에서 직접 읽어 측정했습니다(추정치 아님). GPT 비용은 이 토큰 수 × [OpenAI 공식 단가](https://developers.openai.com/api/docs/pricing)(gpt-4o-mini 입력 $0.15·출력 $0.60 / 1M, gpt-5.5 입력 $5.00·출력 $30.00 / 1M — 2026-07-20 확인, 근거: [references/llm_pricing.md](references/llm_pricing.md))로 계산했습니다. EXAONE은 로컬(Ollama) 구동이라 비용은 항상 $0입니다.

| 케이스 | GPT-5.5 (route+answer) | EXAONE (route+answer) |
|---|---|---|
| 단기리스 | 2,528 토큰 · $0.0226 | 4,011 토큰 · $0(로컬) |
| 파생상품 | 2,815 토큰 · $0.0194 | 4,705 토큰 · $0(로컬) |
| 틀린 전제 | 2,550 토큰 · $0.0217 | 2,250 토큰 · $0(로컬) |
| 근거 없음(미국세법) | 1,856 토큰 · $0.0108 | 1,709 토큰 · $0(로컬) |
