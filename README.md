...
→ 실제로 어떻게 동작하는지는 [3. 실제 동작 예시](#3-실제-동작-예시)에서 확인할 수 있습니다.
 ...
 **• 노출 조건:** 리랭커 점수가 0.6 이상인 관련 사례가 발견되는 경우에만, 최종 답변과 나란히 배치하여 사용자가 비교해 볼 수 있는 '참 고 정보' 형태로만 화면에 제공합니다.<br>
...
→ 세부 수치와 방법론은 [5. 성능 평가 (RAGAS)](#5-성능-평가-ragas)에서 확인할 수 있습니다.
...
- **구어체 질문 검색 실패 — rewrite 정규화 추가**: "개발비의 자산인식요건 알려줘"처럼 붙여쓰거나 구어체인 질문은 정답 문단의 리랭커 점수가 0.608에서 0.012로 급락해 검색에서 통째로 탈락했습니다. 매 질의를 검색 친화적으로 다시 쓰는 rewrite 단계를 추가해 점수를 0.576까지 회복시켰습니다. **(오픈소스 리랭커 모델의 구조적 한계 — 토크나이저로 실측 확인)**: 서브워드 토크나이저가 "자산인식요건"처럼 붙여 쓴 단어를 자·산·인·식·요·건 여섯 조각으로 잘게 쪼개면서, 정규화된 질문·기준서 원문이 공유하던 "▁인식"이라는 온전한 토큰 하나가 사라져버립니다. 리랭커는 질문·문서가 토큰을 얼마나 공유하는지로 점수를 매기므로, 핵심 단어의 토큰이 통째로 사라지면 점수가 급락합니다. 한국어처럼 띄어쓰기가 유동적인 언어에서 서브워드 토크나이저 기반 모델 전반이 갖는 구조적 약점이라, rewrite 정규화는 임시방편이 아니라 이 근본 원인에 정확히 대응한 조치였습니다.
...
- **로컬 서빙 제약에 맞춘 리랭커 모델 선택**: 벤치마크 최고점 리랭커(Gemma2-9B 기반)는 fp16 가중치만으로도 약 18GB라 이 맥미니의 통합메모리 16GB를 그 자체로 초과합니다. 대신 568M 모델(bge-reranker-v2-m3, 가중치 약 1.1GB)을 선택해 임베더와 함께 올려도 실측 피크 메모리 ~3.1GB로 안전 마진을 확보했습니다.
...
- [1. 해결하는 문제](#1-해결하는-문제)
- [2. 주요 특징](#2-주요-특징)
- [3. 실제 동작 예시](#3-실제-동작-예시)
- [4. 기술 스택과 선택 이유](#4-기술-스택과-선택-이유)
- [5. 성능 평가 (RAGAS)](#5-성능-평가-ragas)
- [6. 설치 및 로컬 실행](#6-설치-및-로컬-실행)
- [7. 배포](#7-배포)
- [8. 데이터 출처](#8-데이터-출처)
- [프로젝트 구조](#프로젝트-구조)
...
**대조군으로 직접 검증**: "질문이 조문 표현에 가까우면 exact가 오르는가?"를 실제로 확인하려고, kifrs **공식 용어정의 레코드 393개 전부**(부록A 용어표 + 일부 기준서의 본문 콜론 정의 — 표본을 일부러 뽑지 않고 존재하는 용어 전체)로 **정의조회형 문답셋**을 별도로 만들었습니다.
