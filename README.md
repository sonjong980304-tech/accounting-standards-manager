...
LangGraph 5개 노드가 질문을 처리하고 audit_lookup이 참고 정보(감리지적사례)를 곁들입니다. 검색은 4개 컬렉션(기준서 2 + 질의회신 2, 전부 KASB 자료)으로 나눠 라우터가 필요한 곳만 고르게 했고 SQLite 체크포인터로 대화 맥락을 이어갑니다. 점선으로 표시한 경로는 답변 근거로 쓰이지 않는 **참고 전용** 흐름입니다.
